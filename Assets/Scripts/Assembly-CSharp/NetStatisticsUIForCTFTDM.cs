...
public class NetStatisticsUIForCTFTDM : NetStatisticsUIBase
{
	public class UIPlayerScoreIcon : UIPanelX
	{
		...
		public UINumeric m_secureFlagsNum;

		public UINumeric m_assistFlagsNum;
		...
		public UIPlayerScoreIcon()
		{
			...
			m_secureFlagsNum = new UINumeric();
			m_assistFlagsNum = new UINumeric();
		...
		public void SetAlpha(float alpha)
		{
			...
			m_secureFlagsNum.SetColor(new Color(1f, 1f, 1f, alpha));
			m_assistFlagsNum.SetColor(new Color(1f, 1f, 1f, alpha));
		...
		public void SetParentEx()
		{
			...
			m_secureFlagsNum.SetParent(this);
			m_assistFlagsNum.SetParent(this);
		...
		public override void Draw()
		{
			...
			m_ranking.Draw();
			m_flagIcon.Draw();
			m_secureFlagsNum.Draw();
			m_assistFlagsNum.Draw();
			m_killsNum.Draw();
			m_scoreNum.Draw();
			m_cashNum.Draw();
		}
		...
		public override void Destory()
		{
			...
			if (m_secureFlagsNum != null)
			{
				m_secureFlagsNum.Destory();
			}
			if (m_assistFlagsNum != null)
			{
				m_assistFlagsNum.Destory();
			}
			if (m_killsNum != null)
			{
				m_killsNum.Destory();
	...
	private UIImage secureFlagsTitleImg;

	private UIImage assistFlagsTitleImg;
	...
	public NetStatisticsUIForCTFTDM(UIStateManager stateMgr)
		: base(stateMgr)
	{
	}
	...
	public override void Create()
	{
		...
		{
			rankingTitleImg.AddObject(unitUI, 12, RANK_WIN);
			rankingTitleImg.Rect = rankingTitleImg.GetObjectRect();
			Debug.Log("win");
		}
		else
		{
			rankingTitleImg.AddObject(unitUI, 12, RANK_LOSE);
			rankingTitleImg.Rect = rankingTitleImg.GetObjectRect();
			Debug.Log("lose");
		}
		secureFlagsTitleImg = new UIImage();
		secureFlagsTitleImg.AddObject(unitUI, 12, 32);
		secureFlagsTitleImg.Rect = secureFlagsTitleImg.GetObjectRect();
		assistFlagsTitleImg = new UIImage();
		assistFlagsTitleImg.AddObject(unitUI, 12, 33);
		assistFlagsTitleImg.Rect = assistFlagsTitleImg.GetObjectRect();
		killsTitleImg = new UIImage();
		killsTitleImg.AddObject(unitUI, 12, 34);
		killsTitleImg.Rect = killsTitleImg.GetObjectRect();
		scoreTitleImg = new UIImage();
		scoreTitleImg.AddObject(unitUI, 12, 35);
		scoreTitleImg.Rect = scoreTitleImg.GetObjectRect();
		cashTitleImg = new UIImage();
		cashTitleImg.AddObject(unitUI, 12, 42);
		cashTitleImg.Rect = cashTitleImg.GetObjectRect();
		nextRoundImg = new UIImage();
		nextRoundImg.AddObject(unitUI, 12, 40);
		...
		m_waitingTimeNum.Rect = unitUI.GetModulePositionRect(0, 10, 41);
		players.Clear();
		Sort(player);
		...
		stateMgr.m_UIManager.Add(secureFlagsTitleImg);
		stateMgr.m_UIManager.Add(assistFlagsTitleImg);
	...
	public void InitBluePlayers()
	{
		...
		Rect modulePositionRect = unitUI.GetModulePositionRect(0, 10, 36);
		float num = modulePositionRect.height - 2f;
		float num2 = 0f;
		Rect modulePositionRect2 = unitUI.GetModulePositionRect(0, 10, 24);
		float num3 = 60f;
		for (int i = 0; i < 4; i++)
		{
			...
			{
				flag = true;
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 10, 37);
			}
			else
			{
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 10, 36);
			}
			...
			uIPlayerScoreIcon.m_ranking.AddObject(unitUI, 10, 24 + i);
			...
			if (i < list.Count)
			{
				...
				uIPlayerScoreIcon.m_secureFlagsNum.AlignStyle = UINumeric.enAlignStyle.center;
				uIPlayerScoreIcon.m_secureFlagsNum.SetNumeric(unitUI, 4, UIConstant.FormatNum(list[i].VSStatistics.SecureFlags));
				uIPlayerScoreIcon.m_secureFlagsNum.Rect = new Rect(secureFlagsTitleImg.Rect.x, modulePositionRect.y - num2, secureFlagsTitleImg.Rect.width, modulePositionRect.height);
				uIPlayerScoreIcon.m_assistFlagsNum.AlignStyle = UINumeric.enAlignStyle.center;
				uIPlayerScoreIcon.m_assistFlagsNum.SetNumeric(unitUI, 3, UIConstant.FormatNum(list[i].VSStatistics.AssistFlags));
				uIPlayerScoreIcon.m_assistFlagsNum.Rect = new Rect(assistFlagsTitleImg.Rect.x, modulePositionRect.y - num2, assistFlagsTitleImg.Rect.width, modulePositionRect.height);
				uIPlayerScoreIcon.m_killsNum.AlignStyle = UINumeric.enAlignStyle.center;
				uIPlayerScoreIcon.m_killsNum.SetNumeric(unitUI, 4, UIConstant.FormatNum(list[i].VSStatistics.Kills));
	...
	public void InitRedPlayers()
	{
		...
		int count = list.Count;
		Rect modulePositionRect = unitUI.GetModulePositionRect(0, 10, 38);
		float num = modulePositionRect.height - 2f;
		float num2 = 0f;
		Rect modulePositionRect2 = unitUI.GetModulePositionRect(0, 10, 24);
		float num3 = 60f;
		for (int i = 0; i < 4; i++)
		{
			...
			{
				flag = true;
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 10, 39);
			}
			else
			{
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 10, 38);
			}
			uIPlayerScoreIcon.m_background.Rect = new Rect(modulePositionRect.x, modulePositionRect.y - num2, modulePositionRect.width, modulePositionRect.height);
			uIPlayerScoreIcon.m_ranking.AddObject(unitUI, 10, 24 + i);
			uIPlayerScoreIcon.m_ranking.Rect = new Rect(modulePositionRect2.x, modulePositionRect.y - num2, modulePositionRect2.width, modulePositionRect.height);
	...
	public override bool Update()
	{
		...
		case 1:
		{
			...
			if (GameApp.GetInstance().GetGameWorld().Exit)
			{
				tDMState.AddTotalScore(player.VSStatistics.Score);
			}
			else
			{
				tDMState.AddTotalScore(player.VSStatistics.Score + player.VSStatistics.Bonus);
			}
			subState = 0;
			state = 2;
			break;
