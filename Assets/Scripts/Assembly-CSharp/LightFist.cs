...
public class LightFist : Weapon
{
	...
	public override void DamageBoothByArmor()
	{
		...
		damage *= 1f + player.GetSkills().GetSkill(SkillsType.GLOVE_BOOTH);
	}

	public override float GetDamageAfterBoothByArmor(PlayerSkill skill)
	{
		return base.GetDamageAfterBoothByArmor(skill) * (1f + skill.GetSkill(SkillsType.GLOVE_BOOTH));
	}

	public override void GunOn()
	{
		gun.transform.GetChild(0).GetChild(0).GetComponent<Renderer>().enabled = true;
		gun.transform.GetChild(0).GetChild(1).GetComponent<Renderer>().enabled = true;
	...
	public override void GunOff()
	{
		if (gun != null)
		{
			gun.transform.GetChild(0).GetChild(0).GetComponent<Renderer>().enabled = false;
			gun.transform.GetChild(0).GetChild(1).GetComponent<Renderer>().enabled = false;
	...
	public override void Loop(float deltaTime)
	{
		...
		AudioManager.GetInstance().PlaySoundAt("Audio/specialweapon/energy_glove_fire", player.GetTransform().position);
		...
		GameObject original = Resources.Load("Effect/update_effect/effect_fist_attack_002") as GameObject;
		...
		component.GunType = WeaponType.LightFist;
		...
		{
			PlayerFireRocketRequest request = new PlayerFireRocketRequest(10, gunfire.position, normalized2, trackingID);
