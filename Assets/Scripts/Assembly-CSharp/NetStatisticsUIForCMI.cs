...
public class NetStatisticsUIForCMI : NetStatisticsUIBase
{
	...
	public NetStatisticsUIForCMI(UIStateManager stateMgr)
		: base(stateMgr)
	{
	}
	...
	public override void Create()
	{
		...
		m_waitingTimeNum.Rect = unitUI.GetModulePositionRect(0, 14, 41);
		players.Clear();
		Sort(player);
	...
	public void InitBluePlayers()
	{
		...
		Rect modulePositionRect = unitUI.GetModulePositionRect(0, 14, 36);
		float num = modulePositionRect.height - 2f;
		float num2 = 0f;
		Rect modulePositionRect2 = unitUI.GetModulePositionRect(0, 14, 24);
		float num3 = 60f;
		for (int i = 0; i < 4; i++)
		{
			...
			{
				flag = true;
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 14, 37);
			}
			else
			{
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 14, 36);
			}
			...
			uIPlayerScoreIcon.m_ranking.AddObject(unitUI, 14, 24 + i);
			...
			if (i < list.Count)
			{
				...
				uIPlayerScoreIcon.m_killsNum.SetNumeric(unitUI, 4, UIConstant.FormatNum(list[i].VSStatistics.CMIGiftHit));
	...
	public void InitRedPlayers()
	{
		...
		int count = list.Count;
		Rect modulePositionRect = unitUI.GetModulePositionRect(0, 14, 38);
		float num = modulePositionRect.height - 2f;
		float num2 = 0f;
		Rect modulePositionRect2 = unitUI.GetModulePositionRect(0, 14, 24);
		float num3 = 60f;
		for (int i = 0; i < 4; i++)
		{
			...
			{
				flag = true;
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 14, 39);
			}
			else
			{
				uIPlayerScoreIcon.m_background.AddObject(unitUI, 14, 38);
			}
			uIPlayerScoreIcon.m_background.Rect = new Rect(modulePositionRect.x, modulePositionRect.y - num2, modulePositionRect.width, modulePositionRect.height);
			uIPlayerScoreIcon.m_ranking.AddObject(unitUI, 14, 24 + i);
			uIPlayerScoreIcon.m_ranking.Rect = new Rect(modulePositionRect2.x, modulePositionRect.y - num2, modulePositionRect2.width, modulePositionRect.height);
	...
	public override bool Update()
	{
		...
		case 1:
		{
			...
			if (GameApp.GetInstance().GetGameWorld().Exit)
			{
				cMIState.AddTotalScore(player.VSStatistics.Score);
			}
			else
			{
				cMIState.AddTotalScore(player.VSStatistics.Score + player.VSStatistics.Bonus);
			}
			subState = 0;
			state = 2;
			break;
