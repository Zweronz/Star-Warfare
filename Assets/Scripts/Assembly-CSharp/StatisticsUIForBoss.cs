...
public class StatisticsUIForBoss : StatisticsBaseUI
{
	private const byte SUBSTATE_EXP = 0;

	private const byte SUBSTATE_COMBO = 1;

	private const byte SUBSTATE_CASH = 2;
	...
	private static byte[] PICKUP_TITLE = new byte[2] { 36, 78 };

	private UIImage bossShadowImg;

	private UIImage bossImg;

	private UIImage bossCashImg;

	private UINumeric bossCashNum;

	private UIImage othersCashImg;

	private UINumeric othersCashNum;
	...
	public StatisticsUIForBoss(UIStateManager stateMgr)
		: base(stateMgr)
	{
	}
	...
	public override void Create()
	{
		...
		bossShadowImg = new UIImage();
		bossShadowImg.AddObject(unitUI, 0, 35);
		bossShadowImg.Rect = bossShadowImg.GetObjectRect();
		bossShadowImg.SetSize(new Vector2(modulePositionRect.width, modulePositionRect.height));
		bossImg = new UIImage();
		bossImg.AddObject(unitUI, 0, PICKUP_TITLE);
		bossImg.Rect = bossImg.GetObjectRect();
		bossCashImg = new UIImage();
		bossCashNum = new UINumeric();
		othersCashImg = new UIImage();
		othersCashNum = new UINumeric();
		bossCashImg.AddObject(unitUI, 0, 72 + userState.GetStage() - Global.TOTAL_STAGE);
		bossCashImg.Rect = bossCashImg.GetObjectRect();
		bossCashNum.AlignStyle = UINumeric.enAlignStyle.left;
		bossCashNum.SetNumeric(unitUI, 3, UIConstant.FormatNum(player.GetBossCash()));
		bossCashNum.Rect = unitUI.GetModulePositionRect(0, 0, 38);
		othersCashImg.AddObject(unitUI, 0, 77);
		othersCashImg.Rect = othersCashImg.GetObjectRect();
		othersCashNum.AlignStyle = UINumeric.enAlignStyle.left;
		othersCashNum.SetNumeric(unitUI, 4, UIConstant.FormatNum(player.GetMonsterCash() + player.GetPickupCash()));
		othersCashNum.Rect = unitUI.GetModulePositionRect(0, 0, 40);
		cashImg = new UIImage();
		...
		stateMgr.m_UIManager.Add(bossShadowImg);
		stateMgr.m_UIManager.Add(bossImg);
		stateMgr.m_UIManager.Add(bossCashImg);
		stateMgr.m_UIManager.Add(bossCashNum);
		stateMgr.m_UIManager.Add(othersCashImg);
		stateMgr.m_UIManager.Add(othersCashNum);
	...
	{
		bossShadowImg.Visible = visible;
		bossImg.Visible = visible;
		bossCashImg.Visible = visible;
		bossCashNum.Visible = visible;
		othersCashImg.Visible = visible;
		othersCashNum.Visible = visible;
	}
	...
	public void CalcCash()
	{
		...
		case 1:
			curCash = curMaxCash;
			curMaxCash = player.GetBounsCash();
			bWillStop = true;
