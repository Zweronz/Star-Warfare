...
public class SnowShotScript : MonoBehaviour
{
	...
	private void OnTriggerStay(Collider other)
	{
		...
		SnowGun snowGun = weapon as SnowGun;
		bool flag = false;
		if (!((!isLocal) ? laserTimer.Ready() : snowGun.CoolDown()))
		{
			return;
		}
		if (!isLocal)
		...
		else if (other.gameObject.layer == PhysicsLayer.REMOTE_PLAYER)
		{
			...
			PlayerHitPlayerRequest request = new PlayerHitPlayerRequest((short)damage, int.Parse(enemyByCollider.name), false, 38);
			Debug.Log("phpRequest");
		...
		else if (enemyByCollider.layer == PhysicsLayer.GIFT && GameApp.GetInstance().GetGameMode().ModePlay == Mode.VS_CMI && isLocal)
		{
			...
			PlayerHitItemRequest request2 = new PlayerHitItemRequest((short)damage, idByName, false, 38);
