...
public class NetStatisticsUIForSurvival : NetStatisticsUIBase
{
	...
	public NetStatisticsUIForSurvival(UIStateManager stateMgr)
		: base(stateMgr)
	{
	}
	...
	public new virtual void Create()
	{
		...
		monsterTitleImg.AddObject(unitUI, 7, 23);
		monsterTitleImg.Rect = monsterTitleImg.GetObjectRect();
		pickupTitleImg = new UIImage();
		pickupTitleImg.AddObject(unitUI, 7, 24);
		pickupTitleImg.Rect = pickupTitleImg.GetObjectRect();
		energyTitleImg = new UIImage();
		energyTitleImg.AddObject(unitUI, 7, 25);
		energyTitleImg.Rect = energyTitleImg.GetObjectRect();
		...
		playerFlagImg = new UIImage[num2];
		...
		monsterCashBGImg = new UIImage[num2];
		for (int m = 0; m < num2; m++)
		{
			monsterCashBGImg[m] = new UIImage();
			monsterCashBGImg[m].AddObject(unitUI, 7, 27);
			monsterCashBGImg[m].Rect = new Rect(modulePositionRect3.x, modulePositionRect3.y - (float)(m * num3), modulePositionRect3.width, modulePositionRect3.height);
		}
		monsterCashNum = new UINumeric[num2];
		for (int n = 0; n < num2; n++)
		{
			monsterCashNum[n] = new UINumeric();
			monsterCashNum[n].AlignStyle = UINumeric.enAlignStyle.center;
			monsterCashNum[n].SetNumeric(unitUI, 3, UIConstant.FormatNum(players[n].GetMonsterCash()));
			monsterCashNum[n].Rect = monsterCashBGImg[n].Rect;
		}
		Rect modulePositionRect4 = unitUI.GetModulePositionRect(0, 7, 28);
		pickupCashBGImg = new UIImage[num2];
		for (int num4 = 0; num4 < num2; num4++)
		{
			pickupCashBGImg[num4] = new UIImage();
			pickupCashBGImg[num4].AddObject(unitUI, 7, 28);
			pickupCashBGImg[num4].Rect = new Rect(modulePositionRect4.x, modulePositionRect4.y - (float)(num4 * num3), modulePositionRect4.width, modulePositionRect4.height);
		}
		pickupCashNum = new UINumeric[num2];
		for (int num5 = 0; num5 < num2; num5++)
		{
			pickupCashNum[num5] = new UINumeric();
			pickupCashNum[num5].AlignStyle = UINumeric.enAlignStyle.center;
			pickupCashNum[num5].SetNumeric(unitUI, 3, UIConstant.FormatNum(players[num5].GetPickupCash()));
			pickupCashNum[num5].Rect = pickupCashBGImg[num5].Rect;
		}
		Rect modulePositionRect5 = unitUI.GetModulePositionRect(0, 7, 29);
		pickupEnergyBGImg = new UIImage[num2];
		for (int num6 = 0; num6 < num2; num6++)
		{
			pickupEnergyBGImg[num6] = new UIImage();
			pickupEnergyBGImg[num6].AddObject(unitUI, 7, 29);
			pickupEnergyBGImg[num6].Rect = new Rect(modulePositionRect5.x, modulePositionRect5.y - (float)(num6 * num3), modulePositionRect5.width, modulePositionRect5.height);
		}
		pickupEnergyNum = new UINumeric[num2];
		for (int num7 = 0; num7 < num2; num7++)
		{
			pickupEnergyNum[num7] = new UINumeric();
			pickupEnergyNum[num7].AlignStyle = UINumeric.enAlignStyle.center;
			pickupEnergyNum[num7].SetNumeric(unitUI, 3, UIConstant.FormatNum(players[num7].GetPickupEnegy()));
			pickupEnergyNum[num7].Rect = pickupEnergyBGImg[num7].Rect;
		}
		Rect modulePositionRect6 = unitUI.GetModulePositionRect(0, 7, 30);
		bonusBGImg = new UIImage[num2];
		...
		totalCash = userState.GetCash() + player.GetMonsterCash() + player.GetBounsCash() + player.GetPickupCash();
		...
		stateMgr.m_UIManager.Add(energyTitleImg);
	...
	public void CalcCash()
	{
		...
		case 2:
			curCash = curMaxCash;
			SetPickupEnergyVisible(true);
			curMaxCash = player.GetMonsterCash() + player.GetPickupCash();
			bWillStop = true;
			curMarqueeIndex = curMaxCash.ToString().Length;
			PlayMarquee();
		...
		case 3:
			curCash = curMaxCash;
			curMaxCash = player.GetMonsterCash() + player.GetBounsCash() + player.GetPickupCash();
			bWillStop = true;
			curMarqueeIndex = curMaxCash.ToString().Length;
			PlayMarquee();
	...
	public bool UpdateCash()
	{
		...
		if (subState == 1 && timeSpan.TotalSeconds < 1.0)
		{
			return false;
		}
		int num = 9 - curMarqueeIndex;
